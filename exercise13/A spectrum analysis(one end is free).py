...
class propagate():
    """
    A spectrum analysis of waves on a string in which one end is free to move 
    while the other is held fixed.Assume an initial Gaussian wavepacket located
    ...
    def iteration(self):
        ...
        for n in range(self.time-2):
            for i in range(1,self.steps):
                ...
                self.y[n+2][0]=0
                self.y[n+2][self.steps]=(2-r**2)*self.y[n+1][self.steps]-self.y[n][self.steps]+r**2*self.y[n+1][self.steps-1]
